...
- Declare exactly one top-level `sceneTransitions[]` record for every adjacent pair. Author `intent` and `rationale`, normally letting the compiler choose the registered animated recipe. Use `impact-cut` only when abruptness is itself the approved story beat. Every animated boundary must be opaque and covered by both the outgoing tail and incoming narration lead.
