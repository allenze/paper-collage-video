...
7. Validate a request with `provider:request validate` before invoking it.
   ...
   - A rejected output may become a source only through `provider:recover-rejected-source`: run `--check` first, require the unchanged historical request/output, consumed rejected attempt, declared source SHA, and passing observed key-plane evidence, then record lifecycle `recovery-source` without changing or appending the attempt ledger. It remains a rejected attempt and is never reclassified as succeeded.
...
9. Generate/import one narration file per scene so revisions stay local. Before each production voice call, add `timingBinding` (storyboard scene id, allowed min/max media duration) to the request — `provider:record` measures the take and rejects an unfit one before assembly.
   ...
   **Motif fields and state sequences**
   - A compiled motif field needs one matching deterministic `motif-field` node with the same preset/distribution/count/cycles/bounds/`exclusionZones`; use `rise-drift` for bottom-to-top bubbles.
