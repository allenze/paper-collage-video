...
7. Keep manifest-v4 lifecycle truthful. New records are `active`; replacements preserve the prior record as `superseded`. Mark a human-rejected output or context-only recovery sheet with `project:asset-lifecycle` instead of deleting its provenance or passing it for convenience. `project:quality` keeps those records in `assetHistory` but excludes them from the current pass denominator unless the execution tree still reaches the file. Run `project:quality prepare`, inspect original-resolution files plus generated alpha/checkerboard/motion-stress/semantic-target evidence, and generate a scaffold. Parallax rigs and motif fields are composite targets: review depth order/camera coupling/group stability and field density/bounds/exclusions/motion/looping respectively. The scaffold writes current evidence fingerprints back to the report, so `record-batch` normally needs only reviewer, pass/fail checks, and note; include `evidenceFiles` only to override the inherited set. Do not pass a semantic check merely to unblock production. A hard-alpha `key-edge-clean` result proves only that no soft matte contamination was detected; it does not prove silhouette fidelity, identity distinctness, mechanism correctness, or subject completeness.
