...
description: Initialize, create, resume, revise, or productize editable Remotion paper-collage videos with actual-audio edit points, reusable editorial typography, semantic annotations, data SVG, three-format responsive directing, advanced match transitions, rhythmic storyboards and cuts, camera-coupled depth parallax, deterministic motif fields, persistent visibility events, intent-routed opaque scene transitions, registered supported-subject families, registered limited-animation state sequences, batched pose-sheet generation, deterministic low-alpha rectangular-residue detection, semantic identity/mechanism/diagram contracts, layered keyframe motion, audiovisual events, proof-time validation, dead-air continuity checks, budgeted providers, dual-scope quality review, and local final delivery. Use for paper-cutout, hand-drawn explainer, historical collage, layered illustration, parallax explainer, functional-object diagrams, recurring-character stories, or an interrupted project that has production.json.
---
...
4. Author one schema-v9 storyboard input with a whole-film arc, shared visual language, one scene record per planned scene, one v9 `editorial` authoring contract, and exactly one `sceneTransitions[]` boundary for every adjacent pair. Read `editorial-system-v9.md` when the film uses actual-audio edit points, reusable typography, annotations, data SVG, responsive plans, or advanced transitions; never substitute estimated narration or TTS latency for final local-audio timing. For each boundary, declare the narrative `intent` and concrete `rationale`; normally omit `treatment` so `project:storyboard` deterministically routes `continuity`, `location-change`, `time-passage`, `focus-reveal`, or `chapter-reset` to a registered animated paper transition. Use `intent=impact` plus `treatment.motivation=impact` for an impact cut. An ordinary narrative intent may instead use `treatment={type:"cut", motivation:"rhythmic", durationSeconds:0, beatId}` when the beat is in the outgoing final 20% or incoming first 20%. Classify each beat's visible change and author orthogonal `treatments` for motion, persistent visibility, composition relationship, graphic mechanism, semantic risk, importance, and necessity. Use `depth-parallax`/`parallax-camera` for actual camera-coupled depth and `decorative-field`/`motif-field` for bounded repeated decoration; each motif treatment declares normalized `bounds` plus explicit `exclusionZones` for titles, faces, and explanatory data, and all fields in one scene stay within the 192-instance aggregate budget. Do not emulate either primitive with piles of hand-authored image nodes. Do **not** hand-author `compositionPlan`, resolved edit points, responsive/transition plans, directing summaries, fingerprints, style-proof plans, or sheet grids; `project:storyboard` compiles those deterministically, checks the selected production profile's motion budget, and rejects cheap-transform substitutions for real pose changes. Every scene still needs at least three ordered beats and three proof moments including a final state after `at=0.82`; every proof declares `stateAssertions`, and every beat declares `proofTimeId` or `null`.
...
At `style-review`, create the fewest representative source families needed by the compiler-owned `styleProofPlan` and only enough fictional speech to judge the voice. Run `style:proof`; it must cover the plan's highest semantic-risk classes, each concrete coupled relationship, and state-sequence behavior instead of mechanically selecting one highest score. A low-risk film with none of those facets still gets one `baseline:representative` target. One master may cover several risks when the plan records that reuse. Bind every target and the complete plan fingerprint, then render 3–5 seconds through real v9 nodes. Every selected target, including a `free` target, must produce a non-empty structured composite with current full-frame/crop/debug evidence. Registered or semantic targets also require their quality-compatible member evidence and recorded checks. Inspect full-resolution relationship crops, alpha masks, checkerboard isolates, tight crops, and motion-stress sheets where applicable. Show provider/model, voice identity, sample artifacts, and known cost. After explicit approval, run:

Before any style image call, classify its semantic risk. If it is not decorative, author and lock `semantic-contracts.json` as described in `semantic-contracts.md`; do not treat the prompt as the contract. Use schema-v6 image requests with an explicit output surface and reserve quota-consuming attempts before invoking a host image provider.
...
4. For every coupled group, generate or import one complete master. For `supported-subject`, author one `registered-family.schema.json` file and run `npm run assets:derive-registered-family -- <spec>`; it deterministically derives `support-rear`, `subject`, and `support-front`, preserves one complete canvas/origin, records role/slot/source/mask/clip/provenance/fingerprint/lifecycle in manifest v4, and can patch the existing group nodes. A registered sheet or its formally processed member may be a source. A tight independent image without an explicit placement is rejected and must never be mislabeled as a shared registered canvas. The recovery policy is local reprocessing first, then a masked edit against the complete source context, then complete-source regeneration; isolated member generation is forbidden.
...
9. Run `project:composition-proof`; it synchronizes real narration duration, reuses only evidence whose project, assets, registered-family derivations, proof/render sizes, and runtime-build fingerprints are current, renders changed relationship and semantic-contract evidence, and creates alpha/checkerboard/tight-crop/motion-stress plus alpha-band JSON/overlay evidence for post-style coupled assets. Use `--force` only when a complete frame/composite/evidence rerender is required; the report must record `cache.forced=true`. Then run `project:quality scaffold --output=projects/<slug>/quality-review-scaffold.json --reviewer=<reviewer>`, inspect every suggested evidence file, fill pass/fail decisions and notes, and record the edited scaffold with `record-batch`. Never treat the scaffold as an approval.
