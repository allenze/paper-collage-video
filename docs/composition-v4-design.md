...
| `project:style-proof` | use actual v4 groups, emit current style fingerprints, full-resolution proof/crop/debug frames, and per-member alpha/checkerboard/tight/motion-stress evidence; style approval fails closed when evidence or semantic review is missing |
