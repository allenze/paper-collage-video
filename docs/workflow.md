...
计划完成后，Codex 先为每个节拍判断可见变化，并用 `project:storyboard` 锁定 schema-v5 `treatments` 与相邻场景边界。动作、持久显隐、组合关系、图形机制和语义风险是正交维度：姿态/道具状态变化路由到 `state-sequence`，环境呼吸和镜头变化路由到连续变换，show/hide 路由到持久可见性事件，接触与共享边界分别路由到注册组合，问号/圆圈/箭头路由到可编辑图形。命令确定性编译 `compositionPlan`、风险排名、姿态母版网格、成本摘要和指纹；输入不能手写这些派生字段。每个节拍显式声明 proof 绑定或 null；必需动作超出档位时阻断并要求提高档位或缩小故事范围，不会偷偷降级为位移。它不增加审批次数，而是与叙事、事实、制作档位/预算和 text/image/voice provider 一起由人一次确认。`project:confirm-concept` 批量写入 provider 选择并记录 `capabilities-ready`、`brief-ready`、`approve-concept`，直接进入 `style-review`。

## 2. 风格与虚构音色确认

只生成一张代表性样张和足够判断的短试听。样张生图前先把人物身份、复杂拓扑、功能机构和说明图分类并锁定通用语义契约；宿主生图预留真实尝试额度。`style:proof` 自动选择编译器排名最高的风险 treatment，渲染一段 3–5 秒真实 v6 组合 proof，并绑定导演指纹；不再默认取第一个可用镜头。耦合证明包同时包含原分辨率关系裁切、逐成员 alpha、棋盘格孤立图、紧裁图和相对位移 stress sheet；拓扑相关语义检查必须引用这些证据。`approve-style-voice` 会拒绝缺失、过期或仍待审核的证明，但不会增加第四个人工等待节点。人批准且证明通过后进入批量生产；真人声音克隆需要单独的授权与合法参考材料。
...
该命令依次同步真实旁白时长、生成/导入字幕时间、执行音频-only LUFS/真峰预检、核对故事板蓝图/v6 组合/状态序列/关键帧/事件/场景交接、核验组合证明指纹、执行资产与组合双质量门并推进到 `preview`。在 `preview` / `human-review` 阶段重复执行会做幂等复核而不再次 advance。随后 `project:preview` 渲染半尺寸预览、技术报告、证明时刻联系表和转场联系表；视觉和音频指纹都不变时复用 artifact，只改音频时复用视频流并重新混音/封装，任何视觉指纹变化都强制完整渲染。
